...
REM Step 1: Deploy the database indexes migration
echo 📈 Deploying database indexes migration...
supabase db push

REM Step 2: Verify indexes were created
echo ✅ Verifying indexes were created successfully...
supabase db shell --command "SELECT schemaname, tablename, indexname, pg_size_pretty(pg_relation_size(indexrelid)) as index_size FROM pg_indexes WHERE indexname LIKE 'idx_%%' AND schemaname = 'public' ORDER BY tablename, indexname;"

REM Step 3: Check index usage statistics
echo 📊 Checking index usage statistics...
supabase db shell --command "SELECT schemaname, tablename, indexname, idx_scan, idx_tup_read, idx_tup_fetch FROM pg_stat_user_indexes WHERE indexname LIKE 'idx_%%' ORDER BY idx_scan DESC LIMIT 10;"

REM Step 4: Test a sample query to verify performance
echo 🧪 Testing sample query performance...
supabase db shell --command "EXPLAIN (ANALYZE, BUFFERS) SELECT COUNT(*) FROM listings WHERE is_active = true AND latitude IS NOT NULL AND longitude IS NOT NULL;"
